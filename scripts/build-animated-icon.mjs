...
if (!srcArg || !outArg) {
  console.error("Usage: node scripts/build-animated-icon.mjs <source.gif> <output-transparent.gif>");
  process.exit(1);
}

const srcPath = path.resolve(rootDir, srcArg);
const outPath = path.resolve(rootDir, outArg);
...
encodeGif(lightFrames, delays, outPath);

console.log(`Built ${lightFrames.length} frames -> ${outPath}`);
